...
class MMDrawBox : MMDrawable
{
    ...
    func draw( x: Float, y: Float, width: Float, height: Float, round: Float = 0, borderSize: Float = 0, fillColor: SIMD4<Float>, borderColor: SIMD4<Float> = SIMD4<Float>(repeating: 0), fragment: MMFragment? = nil )
    {
        ...
        let renderEncoder = fragment == nil ? mmRenderer.renderEncoder! : fragment!.renderEncoder!
        let vertexData = mmRenderer.createVertexData( rect )
    ...
    func drawRotated( x: Float, y: Float, width: Float, height: Float, round: Float = 0, borderSize: Float = 0, fillColor: SIMD4<Float>, borderColor: SIMD4<Float> = SIMD4<Float>(repeating: 0), rotation: Float )
    {
        ...
        let vertexData = mmRenderer.createVertexData( MMRect(x - borderSize - border / 2, y - borderSize, width + 2*borderSize, height + 2*borderSize, scale: scaleFactor ) )
        renderEncoder.setVertexBytes(vertexData, length: vertexData.count * MemoryLayout<Float>.stride, index: 0)
        renderEncoder.setFragmentBytes(settings, length: settings.count * MemoryLayout<Float>.stride, index: 0)
...
class MMDrawTexture : MMDrawable
{
    ...
    func draw( _ texture: MTLTexture, x: Float, y: Float, zoom: Float = 1, fragment: MMFragment? = nil, prem: Bool = false, round: Float = 0, roundingRect: SIMD4<Float> = SIMD4<Float>(0,0,0,0), globalAlpha: Float = 1)
    {
        ...
        let vertexData = fragment == nil ?
            mmRenderer.createVertexData( MMRect( x, y, width/zoom, height/zoom, scale: scaleFactor ) )
            : fragment!.createVertexData( MMRect( x, y, width/zoom, height/zoom, scale: scaleFactor ) )
    ...
    func drawScaled( _ texture: MTLTexture, x: Float, y: Float, width: Float, height: Float, globalAlpha: Float = 1, fragment: MMFragment? = nil)
    {
        ...
        let vertexData = fragment == nil ?
            mmRenderer.createVertexData( MMRect( x, y, width, height, scale: scaleFactor ) )
            : fragment!.createVertexData( MMRect( x, y, width, height, scale: scaleFactor ) )
...
class MMDrawCustomState : MMDrawable
{
    ...
    func draw( _ state: MTLRenderPipelineState, x: Float, y: Float, width: Float, height: Float, zoom: Float = 1 )
    {
        ...
        let vertexData = mmRenderer.createVertexData( MMRect( x, y, width/zoom, height/zoom, scale: scaleFactor ) )
        renderEncoder.setVertexBytes(vertexData, length: vertexData.count * MemoryLayout<Float>.stride, index: 0)
...
class MMDrawColorWheel : MMDrawable
{
    ...
    func draw( x: Float, y: Float, width: Float, height: Float, color: SIMD4<Float> )
    {
        ...
        let vertexData = mmRenderer.createVertexData( MMRect( x, y, width, height, scale: scaleFactor ) )
        renderEncoder.setVertexBytes(vertexData, length: vertexData.count * MemoryLayout<Float>.stride, index: 0)
...
class MMDrawArc : MMDrawable
{
    ...
    required init( _ renderer : MMRenderer )
    {
        let function = renderer.defaultLibrary.makeFunction( name: "m4mArcDrawable" )
        state = renderer.createNewPipelineState( function! )
        mmRenderer = renderer
    }
