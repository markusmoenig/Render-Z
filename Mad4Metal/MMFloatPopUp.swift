...
class MMFloatPopUp : MMWidget
{
    ...
    init(_ view: MMView, range: SIMD2<Float> = SIMD2<Float>(0,1), value: Float = 0)
    {
        self.range = range
        self.value = value
        super.init(view)
    ...
    override func draw(xOffset: Float = 0, yOffset: Float = 0)
    {
        ...
        mmView.drawBox.draw( x: rect.x, y: rect.y, width: offset, height: itemHeight, round: 0, borderSize: 1, fillColor : SIMD4<Float>( 0.4, 0.4, 0.4, 1), borderColor: skin.borderColor )
